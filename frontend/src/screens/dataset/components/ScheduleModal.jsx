...
import {
  getSchedulesJsonSchema, postSchedule, putSchedule, deleteSchedule,
} from '../../../Api';
...
function ScheduleModal({
  ...
  const [refreshSchedulesJsonSchema, setRefreshSchedulesJsonSchema] = useState(true);
  const [schedulesJsonSchema, setSchedulesJsonSchema] = useState([]);
  ...
  useEffect(() => {
    if (type === UPDATE_TYPE && editedSchedule !== null) {
      form.setFieldsValue(editedSchedule);
      setSelectedTrigger(trigger);
    }
  }, [type, UPDATE_TYPE, editedSchedule]);

  useEffect(() => {
    if (refreshSchedulesJsonSchema) {
      getSchedulesJsonSchema()
        .then((response) => {
          if (response.status === 200) {
            setSchedulesJsonSchema(response.data);
            console.log(response.data);
            setRefreshSchedulesJsonSchema(false);
          } else {
            message.error('An error occurred while retrieving schedules schema.', 5);
          }
        });
    }
  }, [refreshSchedulesJsonSchema, setRefreshSchedulesJsonSchema]);

  const createOrUpdateScheduleRequest = async (payload) => {
    if (type === CREATE_TYPE) {
      return postSchedule(payload).then((response) => response);
    }
    return putSchedule(payload, editedSchedule.key).then((response) => response);
  };
  ...
  const buildScheduleForm = () => {
    ...
    const ignoredFormItem = ['key', 'create_date', 'created_by', 'modified_date', 'datasource_name', 'trigger', 'description'];

    if (schedulesJsonSchema) {
      console.log(schedulesJsonSchema);
      return schedulesJsonSchema.map((formItemObj) => {
        if (selectedTrigger === formItemObj.title) {
  ...
  const getTriggerOptions = () => {
    ...
    return schedulesJsonSchema.map((item) => {
    ...
    >
      ...
      >
        <Form.Item
          name="trigger"
          label="Trigger"
          rules={[{ required: true, message: 'Select an trigger.' }]}
        >
          <Select
